...
export function encryptEnvelope({ subject, body, recipientKey, senderIdentity }) {
  ...
  const encryptedSubject = nacl.box(util.decodeBase64(toUtf8Base64(subject)), nonce, recipientPublicKey, ephemeral.secretKey);
  const encryptedBody = nacl.box(util.decodeBase64(toUtf8Base64(body)), nonce, recipientPublicKey, ephemeral.secretKey);
  ...
  const signature = nacl.sign.detached(util.decodeUTF8(signaturePayload), util.decodeBase64(senderIdentity.privateSigningKey));
...
export function decryptEnvelope({ envelope, privateEncryptionKey, senderPublicSigningKey }) {
  ...
  const subjectBytes = nacl.box.open(util.decodeBase64(envelope.encryptedSubject), nonce, util.decodeBase64(envelope.ephemeralPublicKey), util.decodeBase64(privateEncryptionKey));
  const bodyBytes = nacl.box.open(util.decodeBase64(envelope.encryptedBody), nonce, util.decodeBase64(envelope.ephemeralPublicKey), util.decodeBase64(privateEncryptionKey));
  if (!subjectBytes || !bodyBytes) throw new Error('Falha ao descriptografar');
